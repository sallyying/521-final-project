...
```{r EDA}
train=paintings_train[,-which(names(paintings_test) %in% c("sale","lot","subject","author"))]%>%
  mutate(price= as.numeric(price))%>%
  mutate(authorstandard=as.factor(authorstandard))

test=paintings_test[,-which(names(paintings_test) %in% c("sale","lot","subject","price","logprice"))]
summary(train)
attach(train)
par(mfrow = c(2,2))
ggpairs(train[,-which(names(train) %in% c("authorstandard","winningbidder"))])

#authorstandard reoccurance
n_occur <- data.frame(table(train$authorstandard))
#gives you a data frame with a list of ids and the number of times they occurred.

n_occur[n_occur$Freq > 1,]
#tells you which authors occurred more than once.
train[train$authorstandard %in% n_occur$Var1[n_occur$Freq > 1],]
