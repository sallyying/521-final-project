...
data.train = paintings_train %>% mutate( # Painting physical features
                                         ...
                                         # Buyer Characteristics
                                         High_Roller = ifelse(winningbidder %in% unlist(HighRollers), 1, 0),
...
randomForest(as.numeric(data[,10])~ as.numeric(data[,c(42:59)]))
...
```{r LASSO}
library(glmnet)
lasso = cv.glmnet(y = data.train[,1], 
                  x = data.train[,-1]%>%as.matrix(),
                       standardize=TRUE, # Scaling
                       alpha = 1)
