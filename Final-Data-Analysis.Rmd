...
#Look at price in relation to some of the paintings physical features
...
data.train = paintings_train %>% mutate( # Painting physical features
                                         ...
                                         endbuyer = ifelse(is.na(endbuyer), "NA", endbuyer)
) %>% select(logprice, Shape, log.Surface, Height_in, Width_in, year, High_Roller, Nfigures, Material, position, endbuyer) %>% na.omit()
...
test.lm = glm(logprice~.^2, data = data.train)
summary(test.lm)

max(paintings_train$winningbidder %in% as.list(HighRollers))

colnames(data)[42:59]

painting.features = glm(logprice~.^2, data = data[,c(10,42:59)])
summary(painting.features)
...
```{r }
library(glmnet)
lasso = cv.glmnet(y = data.train[,1], x = data.train[,-1],
                       standardize=TRUE, # Scaling
                       # lambda=grid, # Using an AutoGrid for now 
                       alpha = 1)
plot(lasso)
