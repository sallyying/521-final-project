...
```{r Reducing the Names}
HighRollers = paintings_train %>% group_by(winningbidder) %>% 
              summarize(count = n(), avg.spend = mean(logprice)/mean(paintings_train$logprice)) %>% 
              filter(count>10) %>% arrange(desc(avg.spend)) %>% head(10) %>%
              select(winningbidder) %>%
              head(10) %>% as.list

# Top 5 Pricey Painters
BigShotPainters = paintings_train %>% group_by(author) %>% 
              summarize(count = n(), avg.haul = mean(logprice)/mean(paintings_train$logprice)) %>% 
              filter(count>10) %>% arrange(desc(avg.haul)) %>%
              select(author) %>%
              head(5) %>% as.list
plot.df = paintings_train %>% 
      mutate(
...
plot1 = ggplot(data = plot.df, 
               aes(x = High_Roller, y = logprice, color = factor(High_Roller)))+
                geom_boxplot() + ggtitle("High Rollers and Log Price")

plot2 = ggplot(data = plot.df, 
                aes(x = BigShotPainter, y = logprice, color = factor(BigShotPainter)))+
                geom_boxplot() + ggtitle("Big-shot Painters and Log Price") 
...
```{r BIC, warning = FALSE, message = FALSE}
...
imputed_Data <- mice(data.test, m=5, maxit = 50, method = 'pmm', seed = 9)
