...
INTRO PARAGRAPH
There are many factors that drive the prices of paintings. Our team wants to find the intrinsic value of paintings then we can judge whether each specific painting on the market has been overvalued or undervalued. In our project, we foucs on the variable selection and tries to detect which variables tend to be informative.
...
2) We also see that some of the dealer of type R has a large range of price and lot than other types.
...
```
Our team finds that type R delaler almost dominates the paintings before year 1776. Type P deal becomes more frequently active from year 1775.
...
```{r BIC}
model1 = lm(logprice ~ . +log(lot) + lot:dealer + log.Surface:Shape + type_intermed:endbuyer + dealer:year, data.train)
...
As is shown in the table, 29 variables are selected by AIC, while 17 variables are selected by BIC. BIC excluded all variables that are excluded by AIC.
...
```
From the residual plot above, we can see that our the BMA model fits the data well. W have a constant residual with mean zero for different fitted prices. Our only concern is that the variance of the residuals seem to differ with different fitted values.


* Variables: must include table of coefficients and CI
```{r Table of Variables and CI}
## Plotting Confidence interval
confint(coef(basg, estimator = "HPM"))
```
...
In our BMA model, our team includes the interaction of Shape and log.Surface.  However, we find that this our BMA does not include this interaction as the coefficient is zero. This could mean that the interactions are not important. 
