...
In terms of variable selection we used BMA which, because it is an average of many models, did not technically reduce the number of variables we used. However, using this approach, we were able to take into account the posterior marginal inclusion probabilities. 

```{r Feature Reduction, echo = FALSE}
library(PCAmixdata)
...
# Most important PCs
word.selection = step(lm(logprice~., data = data.train.features%>%select(-ID)))
...
pred.data$year=as.numeric(pred.data$year)
...
other.col = PCA.scores%>% inner_join(data.train%>% mutate(ID = row_number()), by = "ID") %>% select(dealer)

dims = data_frame(Dim1 = PCA$scores.stand[,1], Dim2 = PCA$scores.stand[,13], 
                  logprice = data.train.features[,1],
                  other.col = other.col[,1]
                  ) %>% filter(Dim1<10 & Dim2<10)

# 3D Scatter
library(scatterplot3d)
scatterplot3d(x = dims$Dim1, y = dims$Dim2, z = dims$logprice, color = as.factor(history), pch = 16, angle = 200)

library(plotly)
p <- plot_ly(dims, x = ~Dim1, y = ~Dim2, z = ~logprice, color=~as.factor(other.col)) %>%
  add_markers() %>%
...
ggplot(data = dims, aes(x = Dim1, y = Dim2, color = other.col)) + geom_point() + ylim(c(-5,5)) + xlim(c(-1,5))
...
log.surface.lm = lm(log.Surface~., data = pred.data)
...
test.imputation$year=as.numeric(test.imputation$year)
...
predictions %>% mutate(ID = row_number()) %>% inner_join(data.val %>% mutate(ID = row_number()), by= "ID") %>% arrange(desc(fit)) %>% select(subject, BigShotPainter.name, High_Roller.name, dealer, year, fit)%>% arrange(desc(fit)) %>% mutate(BigShotPainter.name = substr(iconv(BigShotPainter.name, to="ASCII//TRANSLIT"),1,10),
                                                ...
                                                subject = substr(iconv(subject, to="UTF-8"),1,40))%>%
  head(10) %>% kable(., digits = 0, align = 'c', format = "markdown")
```

From these top paintings, we notice a few common themes. First, we see that the dealer is mostly "R" and 1767 is the most frequently occurring year. Interestingly, we notice that only 4 of the paintings do we have painters (Big Shot Painter name) or bidders (High Rollers) that tend to be associated with high-cost paintings. Finally, we notice that the subject contains some of the words we picked out from our earlier analysis. The words Paysages, Homme, and Femme show up in several of these subject descriptions. 
...
Things learned: 
It's our first time to explore factors that can impact paintings' price. It's normal that the painters' characteristics like name and living status can have impact on their paintings' price. But it's interesting to learn that even the purchasing process can have impact on the paintings' value, like who are the buyer, if there is intermediary and what kind of intermediary. Also, it's funny to see that the physical characteristics like the surface and shape and the size information of the shape can have impact on the paintings' price. But it generally makes sense after giving some thoughts. For example, given other conditions the same, we may find a squre painting more valuable than a painting that is flat and long. We also find that the painting descriptions in the "subject" variable helps to predict the price. Some natural related words (in French expression) like "cows" and "trees" will make the paintings' price higher.  

Further work if time allows:
If we had more time, we would look further into interactions. Also, we would do further work for the "subject" variable. For now we just add variables for words higher than a frequency shreshold. But it might be better if we cluster the features expressed by the words.
