...
### Visualizing some of the words with MCA
```{r Feature Reduction, include = FALSE}
library(PCAmixdata)
...
dims = data_frame(Dim1 = PCA$scores.stand[,1], Dim2 = PCA$scores.stand[,2], 
                  logprice = data.train.features[,1], dealer = PA%>% inner_join(data.train, by = "ID") %>% select(origin_cat)
...
library(plotly)
p <- plot_ly(dims, x = ~Dim1, y = ~Dim2, z = ~logprice, color=~as.factor(dealer)) %>%
  add_markers() %>%
...
```{r BAS prediction, cache = TRUE, echo = FALSE}
library(mice)
