...
set.seed(42)
n = nrow(pred.data)
train.ind = sample(1:n, size=3*n/4, rep=FALSE)
train = as.h2o(pred.data[train.ind,])
test = as.h2o(pred.data[-train.ind,])
```
```{r, echo=TRUE}
set.seed(42)
n = nrow(pred.data)
...
my_gbm <- h2o.gbm(y = "logprice",
                  ...
                  seed = 1)
perf.gbm <- h2o.performance(my_gbm, newdata = test)

# Train & Cross-validate a RF
my_rf <- h2o.randomForest(y = "logprice",
                          training_frame = train,
                          ntrees = 50,
                          nfolds = nfolds,
                          fold_assignment = "Modulo",
                          keep_cross_validation_predictions = TRUE,
                          seed = 1)
perf.rf <- h2o.performance(my_rf, newdata = test)

my_glm <- h2o.glm(y = "logprice",
                          training_frame = train,
                          family="gaussian",
                          nfolds = nfolds,
                          fold_assignment = "Modulo",
                          keep_cross_validation_predictions = TRUE,
                          seed = 1)
perf.glm <- h2o.performance(my_glm, newdata = test)
# Train a stacked ensemble using the GBM and RF above
ensemble <- h2o.stackedEnsemble(y = "logprice",
                                training_frame = train,
                                model_id = "my_ensemble_binomial",
                                base_models = list(my_gbm, my_rf, my_glm))

# Eval ensemble performance on a test set
perf.ensemble <- h2o.performance(ensemble, newdata = test)


c(h2o.mse(perf.glm), h2o.mse(perf.gbm), h2o.mse(perf.rf))
