...
summary(lm(logprice ~ .^2, data=Train%>% select(-authorstyle, -High_Roller, -BigShotPainter, -winningbiddertype)))
```
...
basg=bas.lm(logprice ~ . + lot:dealer + 
      ...
      method="MCMC", MCMC.iterations = 5000000, thin = 20)
...
Plot.df = data_frame(Variable = attr(x, "dimnames")[[1]], 
  ...
  filter(Variable %in% variable.names(BMA))
...
# Set up a new model to predict the residuals from the first model
